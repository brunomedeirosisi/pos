...
import type { PaymentTerm } from '../../types/catalog';
...
export function PaymentTermsPage(): JSX.Element {
  ...
  const createMutation = useMutation({
    mutationFn: (values: FormValues) => paymentTermsService.create(buildPayload(values)),
    onSuccess: () => {
  ...
  function openEditForm(record: PaymentTerm) {
    if (!canWriteCatalog) {
      toast.show(t('common.noPermission'), 'error');
      return;
    }
  ...
  const error = (termsQuery.error as Error | undefined) ?? undefined;
  ...
  const rows = useMemo(() => {
    ...
    return terms.map((term) => (
      <tr key={term.id}>
        <td>{term.name}</td>
        <td>{term.legacy_code ?? '-'}</td>
        <td>
          {canWriteCatalog && (
            <button type="button" className="button secondary" onClick={() => openEditForm(term)}>
              {t('common.edit')}
            </button>
          )}
        </td>
