...
export function ProductGroupsPage() {
    ...
    const createMutation = useMutation({
        mutationFn: (values) => productGroupsService.create(buildPayload(values)),
        onSuccess: () => {
    ...
    const error = groupsQuery.error ?? undefined;
    const rows = useMemo(() => {
        if (!canReadCatalog) {
        ...
        return groups.map((group) => (_jsxs("tr", { children: [_jsx("td", { children: group.name }), _jsx("td", { children: group.legacy_code ?? '-' }), _jsx("td", { children: canWriteCatalog && (_jsx("button", { type: "button", className: "button secondary", onClick: () => openEditForm(group), children: t('common.edit') })) })] }, group.id)));
    }, [groups, t, isLoading, isError, error, canWriteCatalog, canReadCatalog]);
    if (!canReadCatalog) {
        return (_jsxs("div", { className: "card", children: [_jsx("h2", { children: t('productGroups.heading') }), _jsx("p", { children: t('common.noPermission') })] }));
    }
    ...
    return (_jsxs("div", { className: "card", children: [_jsxs("div", { className: "toolbar", children: [_jsx("input", { type: "search", placeholder: `${t('common.search')}...`, value: search, onChange: (event) => setSearch(event.target.value) }), canWriteCatalog && (_jsx("button", { type: "button", className: "button primary", onClick: openCreateForm, children: t('common.add') }))] }), _jsxs("table", { className: "table", children: [_jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { children: t('productGroups.heading') }), _jsx("th", { children: t('products.legacyCode') }), _jsx("th", { children: t('common.actions') })] }) }), _jsx("tbody", { children: rows })] }), _jsx(Modal, { open: modalOpen, onClose: handleModalClose, title: editing ? t('productGroups.editTitle') : t('productGroups.addTitle'), width: "520px", children: _jsxs("form", { onSubmit: onSubmit, children: [_jsxs("div", { className: "form-grid", children: [_jsxs("div", { className: "form-group", children: [_jsxs("label", { htmlFor: "group-name", children: [t('productGroups.heading'), "*"] }), _jsx("input", { id: "group-name", ...form.register('name') }), form.formState.errors.name && (_jsx("small", { style: { color: '#dc2626' }, children: form.formState.errors.name.message }))] }), _jsxs("div", { className: "form-group", children: [_jsx("label", { htmlFor: "group-legacy", children: t('products.legacyCode') }), _jsx("input", { id: "group-legacy", ...form.register('legacy_code') })] })] }), _jsxs("div", { className: "form-actions", children: [_jsx("button", { type: "button", className: "button secondary", onClick: handleModalClose, disabled: isSubmitting, children: t('common.cancel') }), _jsx("button", { type: "submit", className: "button primary", disabled: isSubmitting, children: isSubmitting ? t('common.loading') : t('common.save') })] })] }) })] }));
}
